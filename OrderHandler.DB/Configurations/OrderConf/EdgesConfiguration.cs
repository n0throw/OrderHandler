...
public class EdgesConfiguration : IEntityTypeConfiguration<Edge> {
	public void Configure(EntityTypeBuilder<Edge> builder) {
		builder.HasKey(e => e.Id).HasName("edge_pKey");
		...
		builder.HasOne(d => d.Order)
			.WithOne(p => p.Edge)
			.HasForeignKey<Edge>(d => d.IdOrder)
			.HasConstraintName("edge_idOrder_fKey");

		builder.HasOne(d => d.User)
			.WithMany(p => p.Edges)
			.HasForeignKey(d => d.IdUser)
			.HasConstraintName("edge_idUser_fKey");
	}
}